...
def overlapped_boxes_3d(single_box, box_list):
    ...
    for i, box in enumerate(box_list):
        ...
        buf1 = np.zeros(
            (z_draw_max - z_draw_min, x_draw_max - x_draw_min),
            dtype=np.int32)
...
def overlapped_boxes_3d_fast_poly(single_box, box_list):
    ...
    for i in range(len(box_list)):
        if not non_overlap_mask[i]:
            ...
            intersection = shared_y * shared_area
            union = (y_max - y_min) * area2 + (y0_max - y0_min) * area1
            overlap[i] = np.float32(intersection) / (
                        union - intersection)
    return overlap
...
def bboxes_nms(classes, scores, bboxes, nms_threshold=0.45,
               overlapped_fn=overlapped_boxes_3d, appr_factor=10.0,
               attributes=None):
...
def bboxes_nms_uncertainty(classes, scores, bboxes,
                           scores_threshold=0.25,
                           nms_threshold=0.45,
                           overlapped_fn=overlapped_boxes_3d,
                           appr_factor=10.0,
                           attributes=None):
    """Apply non-maximum selection to bounding boxes.
    """
    ...
    for i in range(scores.size - 1):
        if keep_bboxes[i]:
            ...
            overlaped_bboxes = np.concatenate(
                [bboxes[(i + 1):][valid][remove_overlap], bboxes[[i]]],
                axis=0)
            ...
            scores[i] += np.sum(
                scores[(i + 1):][valid][
                    remove_overlap] * boxes_mean_overlap)
            keep_bboxes[(i + 1):][valid] = np.logical_not(
                remove_overlap)  ##
    idxes = np.where(keep_bboxes)
    classes = classes[idxes]
    scores = scores[idxes]
...
def bboxes_nms_merge_only(classes, scores, bboxes,
                          scores_threshold=0.25,
                          nms_threshold=0.45,
                          overlapped_fn=overlapped_boxes_3d,
                          appr_factor=10.0,
...
def bboxes_nms_score_only(classes, scores, bboxes,
                          scores_threshold=0.25,
                          nms_threshold=0.45,
                          overlapped_fn=overlapped_boxes_3d,
                          appr_factor=10.0,
...
def nms_boxes_3d_uncertainty(class_labels, detection_boxes_3d,
                             detection_scores,
                             overlapped_thres=0.5,
                             overlapped_fn=overlapped_boxes_3d,
                             appr_factor=10.0,
...
def nms_boxes_3d_merge_only(class_labels, detection_boxes_3d,
                            detection_scores,
                            overlapped_thres=0.5,
                            overlapped_fn=overlapped_boxes_3d,
                            appr_factor=10.0,
...
def nms_boxes_3d_score_only(class_labels, detection_boxes_3d,
                            detection_scores,
                            overlapped_thres=0.5,
                            overlapped_fn=overlapped_boxes_3d,
                            appr_factor=10.0,
