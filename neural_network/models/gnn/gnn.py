...
def instance_normalization(features):
    with tf.variable_scope(None, default_name='IN'):
        ...
        features = tf.nn.batch_normalization(
            features, mean, variance, None, None, 1e-12,
            name='IN_apply')
    return (features)
...
def multi_layer_neural_network_fn(features, Ks=(64, 32, 64),
                                  is_logits=False,
                                  normalization_type="fused_BN_center",
                                  activation_type='ReLU'):
                ...
                for class_idx in range(num_classes):
                    with tf.variable_scope('cls_%d' % class_idx):
                        box_encodings = self._loc_fn(
                            ...
                            normalization_type=normalization_type,
                            activation_type=activation_type)
                        box_encodings = tf.expand_dims(box_encodings,
                                                       axis=1)
...
class PointSetPooling(object):
    ...
    def apply_regular(self,
        ...
        point_set_features = tf.gather(point_features,
                                       set_indices[:, 0])
        point_set_coordinates = tf.gather(point_coordinates,
                                          set_indices[:, 0])
...
class GraphNetAutoCenter(object):
    ...
    def apply_regular(self,
        ...
        s_vertex_features = tf.gather(input_vertex_features,
                                      edges[:, 0])
        s_vertex_coordinates = tf.gather(input_vertex_coordinates,
                                         edges[:, 0])
        ...
        d_vertex_coordinates = tf.gather(input_vertex_coordinates,
                                         edges[:, 1])
